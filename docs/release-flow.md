...
Skills live flat at `skills/<name>/` in this repo. Thirteen in-repo skills ship as
...
`update-branch`, `office-hours`, `plan-ceo-review`, `install-skills`, plus deprecated compatibility skills
`superteam` and `superteam-non-interactive`. All thirteen are
...
The `v<X.Y.Z>` ref selects the state of the entire repo at that tag. Because all thirteen
...
- In-repo skills (`scaffold-repository`, `using-github`, `new-branch`,
  `develop-issue`, `finish-pr`, `review-code`, `review-action`, `update-branch`, `office-hours`,
