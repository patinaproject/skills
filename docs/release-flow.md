...
Skills live flat at `skills/<name>/` in this repo. Seven in-repo skills ship as
`patinaproject-skills`: active skills `scaffold-repository`, `using-github`,
`new-branch`, `develop-issue`, `finish-pr`, `review-code`, and `install-skills`. All seven are
...
The `v<X.Y.Z>` ref selects the state of the entire repo at that tag. Because all seven
...
- In-repo skills (`scaffold-repository`, `using-github`, `new-branch`,
  `develop-issue`, `finish-pr`, `review-code`, and `install-skills`) are not separate release-please packages;
