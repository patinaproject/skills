...
#   pnpm sync-pstack
#
# pstack-claude main is the target: every run imports its current tip (not a
...
# raw merge of pstack-claude against our tree has no shared ancestor for these
...
#     merge cleanly takes the renamed pstack-claude content. That is the base switch.
#   - Later runs: the previous carrier commit is the merge base, so the merge
#     writes <<<<<<< / ======= / >>>>>>> markers only where your local edits
#     overlap lines pstack-claude changed since the last sync.
#
...
# 1. Import the current pstack-claude tip.
...
(
  ...
  if git diff --cached --quiet; then
    echo "sync-pstack: carrier already at pstack-claude@$short; nothing new to merge."
  else
    ...
    git commit --quiet --no-verify -m "chore: sync pstack-claude@$short into $DEST"
  fi
)
...
if git merge --no-ff --no-edit "$CARRIER"; then
  echo "sync-pstack: merged pstack-claude@$short cleanly. Review and commit if a merge commit was made."
else
  echo
