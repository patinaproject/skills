...
if [ -f "$WORKFLOW" ]; then
  ...
  assert_match "Do not include exact verification commands, command inventories" "$WORKFLOW"
  assert_match "green check names, or gate inventories when everything passes" "$WORKFLOW"
