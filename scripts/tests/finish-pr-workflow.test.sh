...
if [ -f "$WORKFLOW" ]; then
  ...
  assert_match "Human Relevance Filter" "$WORKFLOW"
  assert_match "Apply this filter to progress reports and final handoffs" "$WORKFLOW"
  assert_match "failed, skipped, interrupted" "$WORKFLOW"
  assert_match "otherwise changes readiness, explains a blocker, identifies residual risk" "$WORKFLOW"
  assert_match "Do not include command inventories, pass counts, green check names, or gate" "$WORKFLOW"
  assert_match "Progress reports should mention the current checkpoint and next action" "$WORKFLOW"
  assert_match "repeated check lists" "$WORKFLOW"
  assert_match "Hide exact verification commands when they all pass" "$WORKFLOW"
