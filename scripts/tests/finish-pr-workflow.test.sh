...
if [ -f "$SKILL" ]; then
  ...
  assert_match "Do not resolve a review thread without an evidence-bearing reply" "$SKILL"
...
if [ -f "$WORKFLOW" ]; then
  ...
  assert_match "never resolve a thread" "$WORKFLOW"
  assert_match "dispositions are eligible only after the fix is present" "$WORKFLOW"
  assert_match "whether the fix covers only the commented line or the broader pattern" "$WORKFLOW"
  assert_match "pattern-based feedback that names a construct" "$WORKFLOW"
  assert_match "an AST query, or a lint rule" "$WORKFLOW"
  assert_match "do not resolve while unexplained matches remain" "$WORKFLOW"
  assert_match "brief current-head evidence reply first" "$WORKFLOW"
  assert_match "report each as resolved after an" "$WORKFLOW"
  assert_match "fixed silently" "$WORKFLOW"
...
if [ -f "$TRIAGE" ]; then
  ...
  assert_match "Every resolved review thread must carry an evidence-bearing reply" "$TRIAGE"
  assert_match "including code-fix dispositions" "$TRIAGE"
  assert_match "Silent resolution is" "$TRIAGE"
  assert_match "For a code-fix disposition, the reply must name what changed" "$TRIAGE"
  assert_match "For pattern-based feedback" "$TRIAGE"
  assert_match "do not resolve while unexplained matches remain" "$TRIAGE"
