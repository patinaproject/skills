...
if jq -e '.skills["code-review"]' skills-lock.json >/dev/null; then
  echo "FAIL: Engineering-owned code-review must not remain in the external skills lock" >&2
  exit 1
fi
