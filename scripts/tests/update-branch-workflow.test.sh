...
if [ -f "$SKILL" ]; then
  ...
  assert_match 'initial update request authorizes' "$SKILL"
  assert_match 'without asking for another confirmation' "$SKILL"
  assert_no_match 'wait for explicit operator confirmation' "$SKILL"
