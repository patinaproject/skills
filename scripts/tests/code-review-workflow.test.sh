...
if [ -f "$WORKFLOW" ]; then
  ...
  assert_match "concurrency:" "$WORKFLOW"
  assert_match "group: code-review-\\$\\{\\{ github\\.event\\.pull_request\\.number \\}\\}" "$WORKFLOW"
  assert_match "cancel-in-progress: true" "$WORKFLOW"
  assert_match "timeout-minutes: [0-9]+" "$WORKFLOW"
  ...
  assert_match "use_sticky_comment: true" "$WORKFLOW"
  assert_match "track_progress: true" "$WORKFLOW"
  assert_match "standalone PR summary comments" "$WORKFLOW"
  assert_match "one final Markdown review summary" "$WORKFLOW"
  assert_match "action-managed sticky comment" "$WORKFLOW"
  assert_match "all actionable line-specific findings inline" "$WORKFLOW"
  assert_match "mcp__github_inline_comment__create_inline_comment" "$WORKFLOW"
  assert_match "--model claude-opus-4-8" "$WORKFLOW"
  assert_match "--effort high" "$WORKFLOW"
  assert_match "--max-turns 100" "$WORKFLOW"
  assert_match "--allowedTools Read,mcp__github_inline_comment__create_inline_comment,Bash\\(gh pr diff:\\*\\),Bash\\(gh pr view:\\*\\),Bash\\(git diff:\\*\\)" "$WORKFLOW"
  assert_no_match "gh pr comment|Bash\\(gh pr comment:\\*\\)" "$WORKFLOW"
