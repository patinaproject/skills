...
if [ -f "$SKILL" ]; then
  ...
  assert_match 'If the review times out or returns a requested partial result' "$SKILL"
  assert_match 'close[[:space:]]+the Explorer or reviewer agent after capturing and reporting the useful[[:space:]]+output' "$SKILL"
  assert_match 'Never close a reviewer before its final report, timeout notice, or[[:space:]]+requested partial result has been captured' "$SKILL"
  assert_match 'Keep only reviewer agents that are[[:space:]]+still actively producing needed results visible' "$SKILL"
