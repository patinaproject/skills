...
if [ -f "$SKILL" ]; then
  ...
  assert_match 'Human Relevance Filter' "$SKILL"
  assert_match 'Apply this filter to progress updates, resumable checkpoints, and final[[:space:]]+handoffs' "$SKILL"
  assert_match 'failed, skipped, interrupted' "$SKILL"
  assert_match 'changes[[:space:]]+readiness, explains a blocker, identifies residual risk, or creates a human[[:space:]]+next action' "$SKILL"
  assert_match 'Do not include command inventories, pass counts, green check names, child-skill[[:space:]]+gate lists' "$SKILL"
  ...
  assert_no_match 'Verified: routine checks passed \(targeted tests, lint, type-check, PR checks\)' "$SKILL"
  assert_no_match 'Verified: routine checks passed; no human action needed before review' "$SKILL"
