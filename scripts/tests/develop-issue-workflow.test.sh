...
if [ -f "$SKILL" ]; then
  ...
  assert_match 'Check for reviewable local changes after verification' "$SKILL"
  ...
  assert_match 'local review status, review finding dispositions,[[:space:]]+blockers, and PR readiness' "$SKILL"
  assert_match 'terminal workflow state' "$SKILL"
  assert_match 'PR ready for human review through[[:space:]]+`finish-pr`' "$SKILL"
  assert_match 'documented human-owned blocker' "$SKILL"
  assert_match 'no reviewable local changes[[:space:]]+and no PR-ready work to publish' "$SKILL"
  ...
  assert_order 'Check for reviewable local changes after verification' 'Delegate final publishing and PR readiness to `finish-pr`' "$SKILL"
  ...
  assert_match 'clean, skipped because no reviewable[[:space:]]+local changes exist, or every local finding has a recorded[[:space:]]+`ready-for-agent`, `ready-for-human`, or `wontfix` disposition' "$SKILL"
  assert_match 'Latest `review-code` result, or that it was skipped because no reviewable[[:space:]]+local changes existed' "$SKILL"
  assert_match 'with the outcome\.' "$SKILL"
  ...
  assert_match 'Verified: targeted tests, lint, type-check, and PR checks passed' "$SKILL"
  assert_match 'Avoid final output shaped like a process transcript' "$SKILL"
  assert_order 'Done: \[#190\]' 'Changed:' "$SKILL"
  assert_order 'Changed:' 'Verified: targeted tests, lint, type-check, and PR checks passed' "$SKILL"
