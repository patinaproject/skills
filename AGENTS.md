...
Require the `Lint` status check before merge. This check runs pull request
metadata linting, including required template checkboxes, conventional PR title
validation, closing-keyword validation, and breaking-change marker consistency.
Configure it in branch protection or a repository ruleset alongside the other
required PR checks.
