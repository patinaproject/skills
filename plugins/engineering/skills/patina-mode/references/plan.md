...
Produce a phased implementation plan grounded in the **Principles** section of the `patina-mode` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `patina-mode` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.
...
- Pass `model:` explicitly per the configured roles (the single-role default in patina-mode's Models section covers code-writing delegations and judgment; multi-model panels run the configured panel — defaults in each panel skill's Models section, overridden via `/setup-engineering`).
...
In the overview, name which patina-mode non-negotiables the implementer must apply, by name:
