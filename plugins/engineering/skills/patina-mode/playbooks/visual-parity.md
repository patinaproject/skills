...
4. Verify each component against its baseline via image diff on the matching surface via the driver skill (`run` for CLIs/TUIs, `verify` for UIs). A nonzero diff is a fail; investigate the pixel delta. `/loop` per component until the diff is zero.
