...
3. Plan the fix. If it crosses a function boundary, `architect` first. Use the **code-review** skill's criteria reference to capture the exact applicable standards and accepted requirements. Include those snapshots and their provenance in the implementation brief. Delegate implementation through provider dispatch using your configured bug-fix descriptor (default `codex:gpt-5.6-sol@max`) with `isolated-write`, a dedicated worktree, and a specific scope; review the diff.
