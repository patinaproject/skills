...
- Repository-documented verification has run and results are recorded. After a
  target merge, use `update-branch`'s applicable-verification contract. A proven
  target-owned failure from an additional broad command is a recorded
  disposition, not a `human-blocked` exit gate.
