...
- Develop an issue end to end: route to the `develop` controller, which
  drives `working-on-github-issue` → build → `polish` → `finish-pr`.
...
- Ready a branch for review (pre-PR gate): route to the `polish` skill —
...
Route to `polish` when the build is complete and the branch needs
readying for review before a PR — it deepens the architecture until settled,
then reviews to green. `polish` runs before `finish-pr`.
