...
- **`/develop`** — The end-to-end controller: `working-on-github-issue` → build →
  `polish` → `finish-pr`, stopping before merge.
- **`/polish`** — The pre-PR readiness gate. It deepens the branch
...
`develop` controller, pre-PR readying to `polish`, editing issues,
