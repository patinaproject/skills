...
`superteam` is a repository-local orchestration skill for running a structured issue workflow across design, planning, execution, review, and finish stages. It is adapted from the Patina Project `super-team` skill, but rewritten to use this repository's public `skills/` and `docs/` layout instead of private Patina paths.
...
- Prefer the repository's normal multi-agent capabilities when available.
